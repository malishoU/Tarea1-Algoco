...
Generadas usando una semilla fija para replicar de forma segura el experimento.

Se fijó la semilla 20260911 al iniciar cada generador; en matrices se fijó
tanto en NumPy como en \texttt{random} con el objetivo de una reproductividad 
del experimento por parte del ayudante. Los datos se repiten ejecutando los
scripts completos en el mismo orden y con las mismas versiones. Todos los
resultados de este informe corresponden a la ultima corrida con estas semillas.
...
El tiempo se midió con el reloj \texttt{steady\_clock} de \texttt{std::chrono}, 
considerando solo el tiempo que uso el algoritmo y dejando afuera lo que es la lectura y 
la escritura de éstos. El uso de la memoria se obtuvo mediante \texttt{getrusage}, en KiB;
corresponde al máximo de memoria utilizada tanto en los datos de las entradas como en la 
memoria utilizada por el mismo algoritmo. Ademas, cada una de las pruebas se ejecuto en 
un proceso independiente. Las funciones de ordenamiento retornan el vector por valor, por 
lo que puede influir un poco en la medición obtenida.

Se usaron límites de 30 s para ordenamiento y 60 s para matrices mediante
\texttt{timeout}. Estos se aplican al proceso completo (incluida la
entrada/salida de cada operación), y no son tiempos exactos del algoritmo. Hubo 11 timeouts en
...
no tres repeticiones de una misma entrada.
